...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            setUser(user);
            if (user) {
                const updatedData = await saveUserToFirestore(user); // Save/update Firestore
                setFirestoreUserData(updatedData); // Set immediately after save
                await fetchUserFromFirestore(user.uid); // Fetch to ensure latest data
            } else {
                setFirestoreUserData(null);
            }
            setLoading(false);
    ...
    // Login Modal component for Florcent & Hampers
    const LoginModal = () => {
        const [isLoading, setIsLoading] = useState(false);

        // Helper function to handle loading state during authentication
        const handleAuthAction = async (authFunction : () => void) => {
            setIsLoading(true);
            try {
                await authFunction();
            } catch {
                ...
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="font-serif text-2xl">
                            {user ? "My Account" : isSignupMode ? "Create Account" : "Welcome Back"}
                        </DialogTitle>
                        {!user && (
                            <CardDescription>
                                {isSignupMode
                            ...
                            <CardContent className="space-y-6 pt-4">
                                <div className="flex items-center gap-4">
                                    <Avatar className="h-16 w-16">
                                            <AvatarImage src={firestoreUserData.photoURL || undefined} alt="Profile" />
                                    ...
                                    <div>
                                        ...
                                        <Badge variant="outline" className="mt-1">Florcent Member</Badge>
                                    </div>
                                </div>
                                ...
                                <div className="text-sm">
                                    <div className="flex items-center gap-2 text-muted-foreground mb-1">
                                        ...
                                        <span>Last login: {new Date(firestoreUserData.lastLogin).toLocaleString()}</span>
                                    </div>
                                </div>
                        ...
                        <div className="space-y-4 py-2">
                            ...
                            {isSignupMode ? (
                                <Button
                                    className="w-full"
                                    disabled={isLoading}
                                    onClick={() => handleAuthAction(() => signUpWithEmail(email, password))}
                                >
                                ...
                                <Button
                                    className="w-full"
                                    disabled={isLoading}
                                    onClick={() => handleAuthAction(() => signInWithEmail(email, password))}
                                >
                                    {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                                    Sign In
                                </Button>
